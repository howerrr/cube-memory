...
2. **雕刻阶段**：在完整的立方体上，点击消除对应的方块
3. **一命制**：每关只有一次机会，消除错误或遗漏任何方块即游戏结束
4. **难度递增**：关卡越高，缺失方块越多，记忆和雕刻时间越短
...
| 记忆时间 | `max(2500, 5000 - level * 250)` | 从 5s 递减，最低 2.5s |
| 雕刻时间 | `max(15000, 30000 - level * 1000)` | 从 30s 递减，最低 15s |
